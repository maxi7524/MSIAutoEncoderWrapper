...
class DatasetCatalog:
    ...
    def get_annotated_spectrum_ids(
        ...
        *,
        ...
        :rtype: List[int]
        """
        clauses = ["sa.source = ?", "sa.dataset_id = ?"]
        parameters: List[Any] = [source, dataset_id]
        if max_fdr is not None:
            clauses.append("a.fdr <= ?")
            parameters.append(float(max_fdr))
        with self.connection() as connection:
            rows = connection.execute(
                "SELECT DISTINCT sa.spectrum_id "
                "FROM spectrum_annotations AS sa "
                "JOIN annotations AS a USING (source, dataset_id, annotation_id) "
                "WHERE " + " AND ".join(clauses) + " ORDER BY sa.spectrum_id",
                parameters,
            ).fetchall()
